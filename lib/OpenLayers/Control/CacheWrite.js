...
OpenLayers.Control.CacheWrite = OpenLayers.Class(OpenLayers.Control, {
    ...
    /**
     * Property: cache
     * {OpenLayers.Control.Cache} The OpenLayers.Contorl.Cache subclass used to
     * store tiles. Default is OpenLayers.Control.Cache.LocalStorage.
     */
    cacheControl: null,
    ...
    initialize: function(options) {
        ...
        if (!options || !options.cacheControl) {
            this.cacheControl = new OpenLayers.Control.Cache.LocalStorage();
        }
    },
     ...
     */
    cache: function(obj) {
        if (this.active && this.cacheControl.isAvailable()) {
            var tile = obj.tile;
            if (tile instanceof OpenLayers.Tile.Image &&
                    tile.url.substr(0, 5) !== 'data:') {
                try {
 ...
 * Clears all tiles cached with <OpenLayers.Control.CacheWrite> from the cache.
 */
OpenLayers.Control.CacheWrite.clearCache = function(cacheControl) {
    if (cacheControl.isAvailable()) {
        cacheControl.clear();
    }
};
