...
OpenLayers.Control.ModifyFeature = OpenLayers.Class(OpenLayers.Control, {
    ...
    initialize: function(layer, options) {
        ...
        this.handlers = {
            ...
            hover: new OpenLayers.Handler.Hover(this, {'move': this.onHoverMove})
        };
    },
    ...
    onHoverMove: function(evt) {
        ...
        if (hits.length > 0) {
            ...
            for (var i=0, ii = hits.length; i<ii; ++i) {
                var hit = hits[i];
                var distance = (centerPt.distanceTo(hit.point1) + centerPt.distanceTo(hit.point2)) / 2;
                if (distance < d) {
                    closestHit = hit;
                }
                d = distance;
            }
            var first = closestHit;
            ...
            createVertex.call(this, first.point1);
            createVertex.call(this, first.point2);
            // create virtual vertex
            var x = (first.point1.x + first.point2.x) / 2;
            var y = (first.point1.y + first.point2.y) / 2;
            var point = new OpenLayers.Feature.Vector(
                new OpenLayers.Geometry.Point(x, y),
                null, this.virtualStyle
            );
            point._sketch = true;
            point._index = first._index;
            point.geometry.parent = first._parent;
    ...
    updateSpatialIndex: function(vertex) {
        ...
        if (hits.length === 1) {
            ...
            bbox = this.createBBOX(hits[0].point1, vertex.geometry);
            bbox.point1 = hits[0].point1;
            bbox.point2 = vertex.geometry;
            bbox._parent = hits[0]._parent;
            bbox._index = hits[0]._index;
            this.tree.insert(bbox);
            bbox = this.createBBOX(hits[0].point2, vertex.geometry);
            bbox.point2 = hits[0].point2;
            ...
            for (i=0, ii=hits.length; i<ii; ++i) {
                ...
                bbox._parent = hit._parent;
                bbox._index = hit._index;
                this.tree.insert(bbox);
    ...
    createBBOX: function(point1, point2) {
        var bbox = [];
        point1.x < point2.x ? bbox.push(point1.x) : bbox.push(point2.x);
        point1.y < point2.y ? bbox.push(point1.y) : bbox.push(point2.y);
        point1.x < point2.x ? bbox.push(point2.x) : bbox.push(point1.x);
        point1.y < point2.y ? bbox.push(point2.y) : bbox.push(point1.y);
        return bbox;
    },
                ...
                if (geometry.CLASS_NAME !== "OpenLayers.Geometry.Point") {
                    ...
                    for (i=0; i<numVert; ++i) {
                        ...
                        if (component.CLASS_NAME == "OpenLayers.Geometry.Point") {
                            var bbox = this.createBBOX(component, geometry.components[i+1]);
                            bbox.point1 = component;
                            bbox.point2 = geometry.components[i+1];
                            bbox._parent = geometry;
                            bbox._index = i+1;
                            data.push(bbox);
                        } else {
