            ...
            if (layer.div.style.display !== "none") {
                if (renderer instanceof OpenLayers.Renderer.Elements) {
                    if (layer instanceof OpenLayers.Layer.Vector) {
                         var hidden_layers = [];
                        // Hide all other visible layers
                        for(j=this.map.layers.length-1; j>=0; --j){
                          if(i!=j && this.map.layers[j].div.style.display !== "none"){
                            hidden_layers.push(this.map.layers[j]);
                            this.map.layers[j].div.style.display = "none";
                          }
                        }
                        ...
                        // Restore layer visibility all other layers
                        for(j=0; j<hidden_layers.length; j++){
                          hidden_layers[j].div.style.display = "block";
                        }
                    }
                    layers.push(layer);
