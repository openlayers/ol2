...
OpenLayers.Format.GML.v3 = OpenLayers.Class(OpenLayers.Format.GML.Base, {
    ...
    writers: {
		...
		if(geometry.pos){
			this.writeNode("gml:pos", geometry.pos, node);		
		}else if(geometry.pointProperty){
			this.writeNode("gml:pointProperty", geometry.pointProperty, node);
		}else if(geometry.pointRep){
			this.writeNode("gml:pointRep", geometry.pointRep, node);
		}else if(geometry.posList){
			this.writeNode("gml:posList", geometry.posList, node);
		}else if(geometry.coordinates){
			this.writeNode("gml:coordinates", geometry.coordinates, node);
		}
                return node;
	    },
