...
OpenLayers.Format.WMC.v1 = OpenLayers.Class(OpenLayers.Format.XML, {
    ...
    write_wmc_Server: function(context) {
        var server = context.server, url,
        node = this.createElementDefaultNS("Server"),
        attributes = {
            service: "OGC:WMS",
        ...
        url = [].concat(server.url);
        for (var i=0,l=url.length;i<l;i++) {
            node.appendChild(this.write_wmc_OnlineResource(url[i]));
        }
        return node;
    },
    ...
    getOnlineResource_href: function(node) {
        ...
        for (var i=0,l=links.length;i<l;i++) {
            this.read_wmc_OnlineResource(object, links[i]);
            out.push(object.href);
        }
