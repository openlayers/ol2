...
OpenLayers.Format.WMTSCapabilities = OpenLayers.Class(OpenLayers.Format.XML.VersionedOGC, {
     ...
     * clientZoomEnabled - {Boolean} True to enable client zoom. 
     *     False by default.
     *
     * Returns:
    ...
    createLayer: function(capabilities, config) {
        var layer, 
            clientZoomEnabled = config.clientZoomEnabled || false;
        delete config.clientZoomEnabled;
        ...
        var resolutions = [], reducedMatrixIds = [], minScaleDenominator, maxScaleDenominator, 
        ...
        for (var j=0, l=tileMatrixSetLinks.length; j<l; j++) {
            ...
            if (tileMatrixSetLink.tileMatrixSet === matrixSet.identifier) {
                if (tileMatrixSetLink.tileMatrixSetLimits) {
                    ...
                    var tmpMatrixIds = {}, mid, scaleDenom;
                    ...
                    for (var k=0, ll=tileMatrixSetLink.tileMatrixSetLimits.length; k<ll; k++) {
                        ...
                        scaleDenom = mid.scaleDenominator;
                        resolutions.push(
                            scaleDenom * 0.28E-3 / OpenLayers.METERS_PER_INCH /
                                OpenLayers.INCHES_PER_UNIT[units]
                        );
                        if (!minScaleDenominator || minScaleDenominator > scaleDenom) {
                            minScaleDenominator = scaleDenom;
                        }
                        if (!maxScaleDenominator || maxScaleDenominator < scaleDenom) {
                            maxScaleDenominator = scaleDenom;
                        }
                    }
                } else {
                    ...
                    for (var k=0, ll=matrixSet.matrixIds.length; k<ll; k++) {
                        resolutions.push(
                            matrixSet.matrixIds[k].scaleDenominator * 0.28E-3 / 
        ...
        if (clientZoomEnabled) {
            options.serverResolutions = resolutions;
        } else {
            options.minScale = 1/Math.ceil(maxScaleDenominator);
            options.maxScale = 1/Math.floor(minScaleDenominator);
        }
