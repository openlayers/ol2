    ...
    createLayer: function(capabilities, config) {
        ...
        for (var mid in matrixSet.matrixIds) {
            if (matrixSet.matrixIds.hasOwnProperty(mid)) {
                var matrix = matrixSet.matrixIds[mid]
                var resolution = matrix.scaleDenominator * 0.28E-3 /
                        OpenLayers.METERS_PER_INCH /
                        OpenLayers.INCHES_PER_UNIT[units]
                resolutions.push(resolution);

                if (!config.maxExtent) {
                    maxExtent.extend(matrix.topLeftCorner);
                    maxExtent.extend(new OpenLayers.LonLat(
                        matrix.topLeftCorner.lon + matrix.matrixWidth *
                            matrix.tileWidth * resolution,
                        matrix.topLeftCorner.lat - matrix.matrixHeight *
                            matrix.tileHeight * resolution));
                }
            }
        }
        ...
        if (layerDef.formats.indexOf('application/utfgrid') < 0) {
            return new OpenLayers.Layer.WMTS(options);
        }
        else {
            return new OpenLayers.Layer.WMTSUTFGrid(options);
        }
    },
