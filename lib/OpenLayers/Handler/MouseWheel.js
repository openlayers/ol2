...
OpenLayers.Handler.MouseWheel = OpenLayers.Class(OpenLayers.Handler, {
     ...
     *    returned from the handler. Default is 6.
     */
    maxDelta: 6,
    ...
    onWheelEvent: function(e){
        ...
        if (!overScrollableDiv && overMapDiv) {
            if (allowScroll) {
                var delta = 0;
                if (!e) {
                    e = OpenLayers.Util.extend({}, window.event);
                }
                ...
                if(this.interval && Math.abs(this.delta) < this.maxDelta) {
                    // Event properties may be gone in IE by the time we
                    // execute the setTimeout function, so we clone the object
