...
OpenLayers.Layer.Bing = OpenLayers.Class(OpenLayers.Layer.XYZ, {
	...
	attributionTemplate : '<span class="olBingAttribution ${type}">' +
	     ...
	     '<img src="${logo}" /></a></div>' +
	     '<div id="vintage"><a target="_blank" href="http://msdn.microsoft.com/en-us/library/ff701712.aspx#sectionSection0">vintageStart</a> : ${vintageStart} <a target="_blank" href="http://msdn.microsoft.com/en-us/library/ff701712.aspx#sectionSection0">vintageEnd</a> : ${vintageEnd}</div>' +
    ...
    /**
     * Property: metadataCenter
     * {Object} Metadata for the center position of the map, as returned by the callback script
     * See http://msdn.microsoft.com/en-us/library/ff701716.aspx Get Imagery data for specific location
     */
    metadataCenter: null,
    ...
    loadMetadata: function() {
		...
		OpenLayers.Util.createScript(this._callbackId,url,true);
    },
    
    /**
     * Method: loadMetadataCenter
     * 
     * Loads Metadata for the center point of the map
     */
     loadMetadataCenter : function () {
		this._callbackId = "_callbackCenter_" + this.id.replace(/\./g, "_");
		window[this._callbackId] = OpenLayers.Function.bind(
		    OpenLayers.Layer.Bing.processMetadata, this
		);
		var params = OpenLayers.Util.applyDefaults({
		...
		var mapCenter=this.map.center.clone().transform(this.map.projection, new OpenLayers.Projection("EPSG:4326"));
		var centerPoint = mapCenter.lat + "," + mapCenter.lon;
		var url = "http://dev.virtualearth.net/REST/v1/Imagery/Metadata/" +
			this.type + "/" + centerPoint +"?" + OpenLayers.Util.getParameterString(params);
		OpenLayers.Util.createScript(this._callbackId,url,true);
    ...
    updateAttribution: function() {
        ...
        this.attribution = OpenLayers.String.format(this.attributionTemplate, {
			...
			vintageStart: this.metadataCenter.resourceSets[0].resources[0].vintageStart,
			vintageEnd: this.metadataCenter.resourceSets[0].resources[0].vintageEnd
        });
...
OpenLayers.Layer.Bing.processMetadata = function (metadata) {
	if (OpenLayers.String.startsWith(this._callbackId,"_callback_"))
	{
	...
	{
		this.metadataCenter = metadata
	}
	OpenLayers.Util.removeDOMElement(this._callbackId);
