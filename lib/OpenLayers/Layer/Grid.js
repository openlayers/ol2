...
OpenLayers.Layer.Grid = OpenLayers.Class(OpenLayers.Layer.HTTPRequest, {
    ...
    initSingleTile: function(bounds) {
        ...
        if(!ignoreMaxExtent) {
            var bottom = Math.max(this.maxExtent.bottom, tileBounds.bottom),
                top = Math.min(this.maxExtent.top, tileBounds.top),
                left = Math.max(this.maxExtent.left, tileBounds.left),
                right = Math.min(this.maxExtent.right, tileBounds.right);
            tileBounds.bottom = Math.min(bottom, top);
            tileBounds.top = Math.max(bottom, top);
            tileBounds.left = Math.min(left, right);
            tileBounds.right = Math.max(left, right);
