...
OpenLayers.Map = OpenLayers.Class({
  ...
  // Options
     ...
     * {Function} The Easing function to be used for tweening.  Default is
     * OpenLayers.Easing.Linear. Setting this to 'null' turns off
     * animated zooming.
     */
    zoomMethod: OpenLayers.Easing.Expo.easeInOut,
  ...
  /********************************************************/
    ...
    zoomTo: function(zoom) {
        if (this.isValidZoomLevel(zoom)) {
            var current = this.getResolution(),
                target = this.getResolutionForZoom(zoom);
            if (this.zoomTween) {
                this.zoomTween.stop();
                this.zoomTween.start({scale: 1}, {scale: current / target}, this.zoomDuration, {
                    callbacks: {
                        eachStep: OpenLayers.Function.bind(function(data) {
                            var containerOrigin = this.layerContainerOriginPx,
                                size = this.size,
                                center = {x: size.w / 2, y: size.h / 2},
                                scale = data.scale,
                                dx = Math.round((scale - 1) * (containerOrigin.x + 50 - center.x)),
                                dy = Math.round((scale - 1) * (containerOrigin.y + 50 - center.y));

                            this.applyTransform(dx, dy, scale);
                        }, this),
                        done: OpenLayers.Function.bind(function(data) {
                            this.applyTransform();
                            this.moveTo(null, this.getZoomForResolution(this.getResolution() / data.scale), true);
                        }, this)
                    }
                });
            } else {
