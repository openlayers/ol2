...
OpenLayers.Map = OpenLayers.Class({
    ...
    zoomTo: function(zoom, xy) {
        ...
        if (this.isValidZoomLevel(zoom)) {
            ...
            if (this.zoomTween) {
                ...
                if (this.zoomTween.playing && this.zoomTween.duration < 3 * this.zoomDuration) {
                    // update the end scale, and reuse the running zoomTween
                    this.zoomTween.finish = {
                        scale: this.zoomTween.finish.scale * end.scale
                    }
                } else {
                    var map = this;
                    this.zoomTween.start(start, end, this.zoomDuration, {
                        callbacks: {
                            eachStep: function(data) {
                                var containerOrigin = map.layerContainerOriginPx,
                                    scale = data.scale,
                                    dx = ((scale - 1) * (containerOrigin.x + 50 - xy.x)) | 0,
                                    dy = ((scale - 1) * (containerOrigin.y + 50 - xy.y)) | 0;
                                ...
                                map.moveTo(map.getLonLatFromPixelAtResolution(xy, resolution), zoom, true);
                            }
                        }
                    });
                }
            } else {
                map.setCenter(map.getLonLatFromPixelAtResolution(xy, map.getResolutionForZoom(zoom)), zoom);
            }
        }
    },
  ...
  //
    ...
    /**
     * Method: getLonLatFromPixelAtResolution
     *
     * Parameters:
     * xy - {<OpenLayers.Pixel>} Current screen location of the zoom origin
     * resolution - {Float} 
     */
    getLonLatFromPixelAtResolution: function (xy, resolution) {
        var size = map.getSize(),
            deltaX  = size.w/2 - xy.x,
            deltaY  = xy.y - size.h/2,
            zoomPoint = map.getLonLatFromPixel(xy);
