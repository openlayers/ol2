   ...
   /** 
    ...
    moveTo: function(lonlat, zoom, options) {
        ...
        if (zoomChanged || centerChanged || dragging) {
            ...
            // Ensure the layer container is not positioned outside the limits of CSS pixel values.
            // https://github.com/openlayers/openlayers/issues/1367
            var isOutsideCssPxLimits = false;
            if (this.layerContainerOriginPx) {
                // Arbitrarily large number that is within the maximum CSS pixel values across major browsers.
                var maxCssPxValue = 1300100;

                isOutsideCssPxLimits = Math.abs(this.layerContainerOriginPx.x) > maxCssPxValue || Math.abs(this.layerContainerOriginPx.y) > maxCssPxValue;
            }

            // (re)set the layerContainerDiv's location
            if (zoomChanged || this.layerContainerOrigin == null || isOutsideCssPxLimits) {
