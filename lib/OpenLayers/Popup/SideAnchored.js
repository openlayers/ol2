...
OpenLayers.Popup.SideAnchored =
  OpenLayers.Class(OpenLayers.Popup, {
    ...
    /**
     * Property: margin
     * {Integer} Amount of pixels that the pop-up will be apart from the
     *     anchor. Useful when the pop-up is triggered from a feature
     *     instead of from a marker.
     */
    margin: 0,
    ...
    */
    ...
    calculateNewPx:function(px) {
        ...
        if (this.relativePosition == "t") {
            top = -size.h - this.anchor.size.h/2 - this.margin;
            left = (this.anchor.size.w - size.w)/2;
        } else if (this.relativePosition == "b") {
            top = this.anchor.size.h*1.5 + this.margin;
            left = (this.anchor.size.w - size.w)/2;
        } else if (this.relativePosition == "l") {
            top = (this.anchor.size.h - size.h)/2;
            left = this.anchor.size.w*1.5 + this.margin;
        } else if (this.relativePosition == "r") {
            top = (this.anchor.size.h - size.h)/2;
            left = -size.w - this.anchor.size.w/2 - this.margin;
        }
