...
OpenLayers.Raster.Composite.fromGridLayer = function(layer) {
    ...
    function update() {
        if (layer.numLoadingTiles == 0) {
            ...
            var cols = canvas.width;
            var data8 = context.getImageData(0, 0, canvas.width, canvas.height).data;
            ...
            if (data8.buffer) {
                ...
                composite.getValue = function(col, row) {
                    var offset = (col + (row * cols));
                    return data32[offset];
                }
            } else if (data8.subarray) {
                // Chrome, Safari, Firefox, ...
                composite.getValue = function(col, row) {
                    var offset = 4 * (col + (row * cols));
                    var components = data8.subarray(offset, offset+4);
                    return components[3] << 24 | components[2] << 16 | components[1] << 8 | components[0];
                }
            } else if (data8.slice) {
                // IE 9, ...
                composite.getValue = function(col, row) {
                    var offset = 4 * (col + (row * cols));
                    var components = data8.slice(offset, offset+4);
                    return components[3] << 24 | components[2] << 16 | components[1] << 8 | components[0];
                }
            } else {
                // fallback
                composite.getValue = function(col, row) {
                    var offset = 4 * (col + (row * cols));
                    var components = Array.prototype.slice.apply(data8, [offset, offset+4]);
                    return components[3] << 24 | components[2] << 16 | components[1] << 8 | components[0];
                }
            }
            composite.events.triggerEvent("update");
